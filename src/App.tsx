...
const NextButton: React.FC<NextButtonProps> = ({ to, disabled, children, onClick, ...rest }) => {
  ...
  <aside className="guide-panel">
    ...
    <ol className="guide-steps">
      <li>
        ...
        <p>
          In the left-hand nav, click <b>Reports</b> → <b>Add New Template</b> →
          choose your object (Address Book, Cases, etc.).
        </p>
      </li>
      <li>
        <strong>Configure the columns</strong>
        <p>
          Switch to the <b>Report Columns</b> tab and add every field your import
          expects. Use these exact headers:
        </p>
      ...
      <li>
        ...
        <p>
          Save the template, click <b>Run</b>, then <b>Export → Excel</b> to download
          your .xlsx.
        </p>
      </li>
      <li>
        ...
        <p>
          ...
          attorney) you may choose to include their data in the excel or fill it later.
        </p>
      </li>
      <li>
        <strong>Merge exports</strong>
        <p>
          You can upload multiple excels and they will be automatically merged.
        </p>
      </li>
    </ol>
  </aside>
...
const UploadPage: React.FC<UploadPageProps> = ({ uploadedFiles, handleFileUpload, handleRemoveFile }) => (
  <div className="upload-container">
    <div className="upload-panel">
      ...
      <div className="generate-button-container">
        <NextButton to="/people" disabled={uploadedFiles.length === 0} />
      </div>
    </div>
    <GuideSection />
  </div>
...
interface PeoplePageProps {
  ...
  handlePersonSelect: (type: keyof PeoplePageProps["selectedPeople"], person: Person) => void;
  handlePersonDataChange: (type: keyof PeoplePageProps["selectedPeople"], data: Record<string, any>) => void;
  onGenerate: () => void;
  isProcessing: boolean;
  isFormComplete: boolean;
}
const PeoplePage: React.FC<PeoplePageProps> = ({ uploadedFiles, selectedPeople, handlePersonSelect, handlePersonDataChange, onGenerate, isProcessing, isFormComplete }) => (
...
const DownloadPage: React.FC<DownloadPageProps> = ({ generatedFiles, onDownloadAll }) => (
  <div className="download-container">
    <div className="right-panel">
      <section className="files-section">
        <div className="files-header">
        </div>
        <GeneratedFiles files={generatedFiles} onDownloadAll={onDownloadAll} />
      </section>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const handleFileUpload = useCallback((files: File[]) => {
    const newFiles = files.map((file) => ({ id: Math.random().toString(36).substr(2), file, name: file.name }));
  ...
  const handlePersonSelect = useCallback(
    (type: keyof PeoplePageProps["selectedPeople"], person: Person) => {
      setSelectedPeople((prev) => ({ ...prev, [type]: person }));
    },
    []
  );
  // ↓↓↓ NEW: receive form-data updates from PeopleSelector ↓↓↓
const handlePersonDataChange = useCallback(
      (type: keyof PeoplePageProps["selectedPeople"], data: Record<string, any>) => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!selectedPeople.plaintiff || !selectedPeople.defendant || !selectedPeople.attorney) {
    ...
    navigate('/download');
  }, [uploadedFiles, selectedPeople, processFiles, setError, navigate, plaintiffData, defendantData, attorneyData]);
    ...
    <div className="App">
      <Routes>
        ...
        <Route
          path="/download"
          element={
            (!uploadedFiles.length || !isFormComplete) ? (
              <Navigate to="/" />
            ) : (
              <DownloadPage
